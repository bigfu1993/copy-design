<template>
    <div :id="id" class="table-wrapper d-flex flex-column">
        <div class="d-flex align-center justify-space-between px-4 table-opts">
            ...
            <div>
                <a-button type="primary" size="small" @click="filterVisible = !filterVisible">高级筛选</a-button>
                        ...
                        <div style="min-width: 120px">
                            ...
                            <div class="d-flex align-center justify-space-between px-2">
                                <a-button class="filter-btn" size="small"
                                    @click="filterColumnsKeys = originColumns.map((c: Column) => c.id)">重置</a-button>
                                <a-button class="filter-btn" size="small" disabled>确定</a-button>
                            </div>
                        </div>
                    </template>
                    <a-button type="primary" size="small" class="ml-4">过滤</a-button>
                </a-popover>
            </div>
        </div>
        ...
        <a-pagination class="pagination" show-quick-jumper show-size-changer showLessItems v-model:current="page"
            v-model:pageSize="pageSize" :total="total" :show-total="(total: any) => `共 ${total} 条记录`"
            @change="(page: number, pageSize: number) => pageChange(page, pageSize)" />
    </div>
</template>
...
import { Action, Column, AnyData } from "./interface";
...
// table主逻辑模块
const columns = ref<Column[]>([]); // 表头数据
const dataSource = ref<Record<string, any>[]>([]); // 表格数据
const selectedRows = ref<Record<string, any>>([]); // 当前选中的行
const queryForm = ref<Record<string, number | string | (number | string)[] | undefined>>({}); // 高级筛选参数

// 原始表头数据
let originColumns = ref<Column[]>([])
// 当前可见的表头（过滤操作checkbox选中值）
let filterColumnsKeys = ref<any[]>([])
// 过滤操作checkbox列表（通过源数据originColumns自动生成 ）
let filterColumns = computed(() => {
    return originColumns.value.map((c: any, i: number) => ({ label: c.title, value: c.id, disabled: c.disabled }))
})
...
let renderColumns = computed(() => {
    ...
    outerKeys.forEach((o: any, i: number) => {
        if (originColumns.value.findIndex((oo: any, ii: number) => oo.id == o) < 0) {
    ...
    return originColumns.value
        .map((c: Column, i: number) => {
            ...
            for (const row of dataSource.value) {
                if (row[c.id]) {
                    if (c.choices) {
                        const selected: any = c.choices.find((choice: any) => choice.key === row[c.id]);
        ...
        .filter((c: Column, i: number) => filterColumnsKeys.value.indexOf(c.id) > -1)
})

// 合并action
let actions = computed(() => {
    let result: any = []
    let defaultMd: any = { height: "20px", width: "20px", text: '', filter: (item: any) => true }
    let defaultEdit: any = { id: "edit", icon: "icon-bianji3", tooltip: "编辑", color: "#1684fc", background: "#e3eefb" }
    let defaultDelete: any = { id: "delete", icon: "icon-shanchu1", tooltip: "删除", color: "#ff2222", background: "#ffe8e8" }
    let editIdx = props.actions.findIndex((k: any) => k.id == 'edit')
    let deleteIdx = props.actions.findIndex((k: any) => k.id == 'delete')
    if (editIdx < 0) {
    ...
    for (const item of columns.value) {
        ...
        if (value) {
            switch (xType) {
                ...
                case "foreign":
                case "select":
                    queryData[key + "__in"] = (value as (string | number)[]).join(",");
                    break;
                case "boolean":
                    queryData[key] = value.toString();
                    break;
                case "number":
                    queryData[key + symbolMap[(value as (string | number)[])[0]]] = (
                ...
                case "time":
                case "datetime":
                    queryData[key + "__range"] = (value as (string | number)[]).join(",");
                    break;
            }
        }
    }
...
async function getColumns() {
    ...
    if (data) {
        ...
        originColumns.value = temp.map((column: any, i: number) => {
...
async function getTableData() {
    ...
    dataSource.value = res.list.map((item: any) => ({
        ...item,
        actions: actions.value.filter((action: any) => {
            return action.filter ? action.filter(item) : true;
        }),
    }));
}
// 操作-删除
async function deleteRows(delIds: any[]) {
    if (!delIds.length) {
...
// 重写datasource
function overwriteDataSource(overwriteDataSource: any[], findFn: any) {
    dataSource.value = dataSource.value.map((d: any, i: number) => {
        let idx = overwriteDataSource.findIndex((dd: any, ii: number) => {
            return findFn(d, dd)
        })
        if (idx > -1) {
...
// 列表复选操作
function rowSelect(rowKeys: number[] | string[], rows: AnyData) {
...
function handleDelete() {
    const delIds = selectedRows.value.map((row: any) => row.id);
    deleteRows(delIds)
}

// 操作
function doAction(action: Action, record: Record<string, any>, column: any, e: Event) {
    if (action.id === "delete") {
        // 内部处理
