...
export default function tableMixin({ getTableData }: any): any {
  ...
  function setTableH() {
    const el: any = document.querySelector(".ant-table-body");
    el.style.height = `${
      innerHeight.value - 80 - (filterVisible.value ? 120 : 0)
    }px`;
  }
  // 表格文本宽度
  function getTextWidth(text: string, font = "13px Microsoft YaHei") {
  ...
  //  切换分页
  async function pageChange(curPage: number, pageSize?: number) {
    page.value = curPage;
    await getTableData();
  }
  onMounted(() => {
